...
namespace cgp{
	...
	void opengl_fbo_structure::shadowInitialize() {

		width = 640;
		height = 480;

		// Initialize texture
		texture.initialize_texture_2d_on_gpu(width, height, GL_DEPTH_COMPONENT, GL_TEXTURE_2D);
		...
		glDrawBuffer(GL_NONE); //permet de specifier à opengl qu'on à rien à foutre 
		glReadBuffer(GL_NONE); //de la couleur
		// associate the depth-buffer
		//glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, depth_buffer_id);
		// Reset the standard framebuffer to output on the screen
		glBindFramebuffer(GL_FRAMEBUFFER, 0);
	...
	void opengl_fbo_structure::shadowUpdate_screen_size(int new_width, int new_height) {
